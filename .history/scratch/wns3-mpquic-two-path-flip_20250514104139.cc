...
main (int argc, char *argv[])
{
    ...
    double rate0a = 5.0; // minimum rate of path 1, zhiy zeng
    double rate1a = 10.0; // minimum rate of path 2, zhiy zeng
    double delay0a = 50.0; // minimum delay of path 1, zhiy zeng
    double delay1a = 10.0; // minimum delay of path 2, zhiy zeng
    double rate0b = 5.0; // maximum rate of path 1, zhiy zeng
    double rate1b = 10.0; // maximum rate of path 2, zhiy zeng
    double delay0b = 50.0; // maximum delay of path 1, zhiy zeng
    double delay1b = 10.0; // maximum delay of path 2, zhiy zeng   
     ...
     */
     ...
     c.Create (10); // create 10 nodes, zhiy zeng
     NodeContainer n0n1 = NodeContainer (c.Get (0), c.Get (1)); // n0 and n1, zhiy zeng
     NodeContainer n1n8 = NodeContainer (c.Get (1), c.Get (8)); // n1 and n8, zhiy zeng
     NodeContainer n8n2 = NodeContainer (c.Get (8), c.Get (2)); // n8 and n2, zhiy zeng
    ...
    // Dynamically change link rate and delay, zhiy zeng
    for (double i = 1.0; i < 5; i = i+0.1){
    ...
    // change link rate and delay, zhiy zeng
    for (double i = 5.0; i < simulationEndTime; i = i+0.1){
        Simulator::Schedule (Seconds (i), &ModifyLinkRate, &d1d8  , DataRate(std::to_string(rateVal1->GetValue())+"Mbps"),  Time::FromInteger(delayVal1->GetValue(), Time::MS));
