...
/**
 * 作用：接收来自 IPv4 层的 ICMP 错误消息；
 * 参数说明：
    icmpSource: 发送 ICMP 消息的源地址；
    icmpTtl: ICMP 报文的 TTL；
    icmpType: ICMP 类型（如 Destination Unreachable, Time Exceeded 等）；
    icmpCode: ICMP 子类型码；
    icmpInfo: 可选信息字段（如 MTU 值）；
    payloadSource: 原始导致 ICMP 的 IP 数据报的源地址；
    payloadDestination: 原始数据报的目标地址；
    payload[8]: 原始 UDP/IP 首部的一部分（通常包含端口号等）；
  * zhiy zeng
 */
void
...
/**
 * 作用：这是一个“伪接收函数”，用于拦截非法调用；
 * 参数说明：
    packet: 接收到的数据包；
    incomingIpHeader: IP 首部信息；
    incomingInterface: 接收该数据包的网络接口；
  * 返回值：总是返回 IpL4Protocol::RX_OK（但实际不会使用）；
  * zhiy zeng
 */
...
/**
 * 作用：将封装了 QUIC 报文的数据包通过 UDP 发送；
 * 参数说明：
    socket：发送数据的 QUIC 套接字；
    pkt：要发送的有效载荷数据包；
    outgoing：QUIC 报头信息（包含路径 ID、包号等）；
 * 返回值：无；
 * zhiy zeng
 */
void
...
/**
 * 作用：从协议栈中移除指定的 QUIC 套接字；
 * 参数说明：
    socket: 要移除的 QUIC 套接字；
 * 返回值：
    true：成功找到并移除了绑定项；
    false：未找到对应的绑定项；
  * zhiy zeng
...
/**
 * 作用：为 QUIC 协议分配一个新的 IPv4 端点；
 * 参数说明：无；
* 返回值：
  成功时返回指向新分配的 Ipv4EndPoint 对象；
  失败时可能返回 nullptr（取决于 m_endPoints->Allocate() 的实现）；
* zhiy zeng
 */
Ipv4EndPoint *
...
/**
 * 作用：为给定的网络设备和端口号分配一个新的 IPv4 端点；
 * 参数说明：
    boundNetDevice: 要绑定的网络设备（如以太网接口、Wi-Fi 接口等）；
    port: 要绑定的本地端口号；
 * 返回值：
    成功时返回指向新分配的 Ipv4EndPoint 对象；
  失败时可能返回 nullptr；
...
/**
 * 作用：为给定的网络设备、IPv4 地址和端口号分配一个新的 IPv4 端点；
 * 参数说明：
    boundNetDevice: 要绑定的网络设备（如以太网接口、Wi-Fi 接口等）；
    address: 要绑定的本地 IPv4 地址；
    port: 要绑定的本地端口号；
 * 返回值：
    成功时返回指向新分配的 Ipv4EndPoint 对象；
    失败时可能返回 nullptr；
...
/**
 * 作用：为给定的网络设备、本地 IP/端口和远端 IP/端口分配一个新的 IPv4 端点；
 * 参数说明：
    boundNetDevice: 要绑定的网络设备（如以太网接口、Wi-Fi 接口等）；
    localAddress: 本地要绑定的 IPv4 地址；
    localPort: 本地要绑定的端口号；
    peerAddress: 远端通信对端的 IPv4 地址；
    peerPort: 远端通信对端的端口号；
 * 返回值：
    成功时返回指向新分配的 Ipv4EndPoint 对象；
    失败时可能返回 nullptr；
...
/**
 * 作用：为 QUIC 协议分配一个新的指定地址的IPv6 端点；
 * 参数说明：无；
* 返回值：
  成功时返回指向新分配的 Ipv6EndPoint 对象；
  失败时可能返回 nullptr（取决于 m_endPoints->Allocate() 的实现）；
* zhiy zeng
 */
Ipv6EndPoint *
...
/**
 * 作用：为给定的网络设备、IPv4 地址和端口号分配一个新的 IPv6 端点；
 * 参数说明：
    boundNetDevice: 要绑定的网络设备（如以太网接口、Wi-Fi 接口等）；
    address: 要绑定的本地 IPv6 地址；
    port: 要绑定的本地端口号；
 * 返回值：
    成功时返回指向新分配的 Ipv6EndPoint 对象；
    失败时可能返回 nullptr；
...
/**
 * 作用：为给定的网络设备、本地 IP/端口和远端 IP/端口分配一个新的 IPv6 端点；
 * 参数说明：
    boundNetDevice: 要绑定的网络设备（如以太网接口、Wi-Fi 接口等）；
    localAddress: 本地要绑定的 IPv6 地址；
    localPort: 本地要绑定的端口号；
    peerAddress: 远端通信对端的 IPv6 地址；
    peerPort: 远端通信对端的端口号；
 * 返回值：
    成功时返回指向新分配的 Ipv6EndPoint 对象；
    失败时可能返回 nullptr；
...
/**
 * 作用：设置一个回调函数，用于当本协议需要向下层（即链路层）发送数据包时调用 (IPv4)；
 * 参数说明：
    callback: 下层目标回调函数，类型为 IpL4Protocol::DownTargetCallback；
 * 返回值：无；
 * zhiy zeng
 */
void
...
/**
 * 作用：获取当前传输层向下发送数据包的目标回调函数 (IPv4)；
 * 参数说明：无；
 * 返回值：
    返回一个类型为 IpL4Protocol::DownTargetCallback 的回调函数对象；
    可用于调用 IP 层的发送逻辑；
 * zhiy zeng
 */
...
/**
 * 作用：设置一个回调函数，用于当本协议需要向下层（即链路层）发送数据包时调用 (IPv6)；
 * 参数说明：
    callback: 下层目标回调函数，类型为 IpL4Protocol::DownTargetCallback；
 * 返回值：无；
 * zhiy zeng
...
/**
 * 作用：为指定的 QUIC 套接字和路径 ID 创建并绑定一个新的 UDP 套接字；
 * 参数说明：
    pathId: 路径标识符，用于区分不同路径；
    socket: 关联的 QUIC 套接字；
    localAddress: 当前路径使用的本地地址；
    peerAddress: 当前路径连接的目标地址；
 * 返回值：
    成功时返回 0；
    失败时返回负值（如 -1 表示错误）；
 * zhiy zeng
...
/**
 * 作用：设置是否允许 QUIC 协议使用 0-RTT 握手；
 * 参数说明： 
    allow0RTT：布尔值，表示是否允许 0-RTT 握手；
    true：允许；
    false：不允许；
 * 返回值：无；
 * zhiy zeng
...
/**
 * 作用：为指定路径 ID 的 UDP 套接字重新调用 Connect(...) 方法；
  重新设置指定路径（path）的 UDP 套接字的目标地址（即连接到新的对端地址）
 * 参数说明：
    pathId: 路径标识符，用于定位特定的 UDP 绑定；
    peerAddress: 新的目标地址（通常是对端的 IP 和端口）；
 * 返回值：
    成功时返回 0；
    失败时返回负值（如 -1 表示未找到对应路径）；
 * zhiy zeng
 */
int
