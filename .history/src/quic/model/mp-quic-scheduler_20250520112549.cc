...
/**
 * zhiy zeng: Peekaboo 调度策略
  * 仅允许两条子流（路径）
  * Peekaboo 是一种基于MAB的调度策略
 */
std::vector<double>
MpQuicScheduler::Peekaboo()
{
  NS_LOG_FUNCTION (this);
  uint8_t K = m_subflows.size(); // 手臂数量 = 子流数量
  if(EPR.size() < K)
  {
    EPR.push_back(0.0); // EPR: Expected Potential Reward（预期潜在收益），用于路径评分
    A.push_back(MatrixXd::Identity(6,6)); // A: 6x6单位矩阵，每个路径的协方差矩阵, 反映历史观测对当前估计的影响
    b.push_back(VectorXd::Constant(6,0)); // b: 6维向量, 观测累积向量
  }
  std::vector<double> tosend(m_subflows.size(), 0.0);
  ...
  if (m_socket->AvailableWindow (fastPathId) > 0){
    ...
    for (int i = 0; i < 2; i++){
      ...
      // peek_x: 6维向量, 用于存储路径的特征值, mp-quic-scheduler.h中定义
    ...
    // 更新所选路径的统计模型（更新 A 和 b）
    A[m_lastUsedPathId] = A[m_lastUsedPathId] + peek_x * peek_x.transpose();
    // R: 在mp-quic-scheduler.h中定义, 初始化为0, 观测噪声项（通常为奖励值）
    // PeekabooReward(uint8_t pathId, Time lastActTime)中更新R和peek_x
...
/**
 * zhiy zeng: PEEKABOO 路径调度算法中用于更新奖励（Reward）
 * 和特征向量（Feature Vector）的函数
 */
void
MpQuicScheduler::PeekabooReward(uint8_t pathId, Time lastActTime)
{
  ...
  T_r = std::max(2*rtt_f, rtt_s); // 理论最大响应延迟，用于判断响应是否及时
  T_e = (Now () - lastActTime).GetMilliSeconds(); // 当前时间与上次操作的时间间隔, 实际响应延迟
  if (T_e < 3 * T_r) // 判断是否处于有效反馈区间，并计算 Reward
    { 
      // 基础吞吐奖励（假设 MSS=1460 字节）
      double r = 1460 * 1000 * 1e9/ (Now() - lastActTime).GetDouble();
      R = R + r * g; // g: 奖励衰减因子
      if (T_e <= T_r) // 响应非常快，奖励折扣较小，保留更多历史
        {
          g = 0.9 * g;
        }
      else if (T_e <= 2 * T_r) // 正常响应，奖励折扣中等
        {
          g = 0.7 * g;
        }
      else // 响应较慢，奖励折扣较大
