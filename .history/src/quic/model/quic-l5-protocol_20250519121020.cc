...
// zhiy zeng
/* 
作用：将传入的数据包发送到指定的 QUIC 流；
参数说明：
data: 待发送的数据包（Ptr<Packet> 是 NS-3 中的数据结构）
streamId: 指定的流 ID
返回值：
成功发送的字节数；
失败时可能返回 -1 或其他负数
...
QuicL5Protocol::DispatchSend (Ptr<Packet> data, uint64_t streamId)
{
  ...
  if (stream->GetStreamDirectionType () == QuicStream::SENDER
    ...
    {
      sentData = stream->Send (data); // 调用流对象的 Send(data) 方法发送数据
    }
...
/* 
作用：接收一个包含多个 QUIC Frame 的 Packet，并将其分发给相应的流或socket 处理；
参数说明：
data: 接收到的数据包；
address: 发送方的地址（用于响应或连接管理）；
返回值：
成功时返回非零值（表示是否需要发送 ACK）；
失败时返回 -1。
zhiy zeng
*/
int
QuicL5Protocol::DispatchRecv (Ptr<Packet> data, Address &address)
{
  ...
  // 判断是否为纯 ACK 数据包
  for (auto &elem : disgregated) // 遍历所有 Frame
    {
  ...
  for (auto it = disgregated.begin (); it != disgregated.end (); ++it)
    {
      ...
      else // 其他帧（如 ACK、CONNECTION_CLOSE）由 socket 层统一处理
        {
          NS_LOG_INFO (
  ...
  return !onlyAckFrames; // 返回是否需要发送 ACK
}

/*
作用：将传入的 Packet（通常是一个 QUIC Frame）放入 socket m_socket的发送缓冲区；
参数说明：
frame: 要发送的数据包（Packet），通常已经封装了 QUIC 子头部（如 STREAM、ACK 等）；
返回值：
返回 m_socket->AppendingTx(frame) 的结果，通常是成功发送的字节数；
如果失败（例如缓冲区满或流控限制），可能返回 -1 或其他负值。
zhiy zeng
*/
int
...
/*
作用：处理一个从网络层接收到的数据包（Packet），并将其交给 QUIC Socket 层
进行进一步解析和处理；
参数说明：
frame: 接收到的数据包（通常是一个完整的 QUIC Frame 或包含多个 Frame 的 Packet）；
address: 发送方的地址（用于回传信息或连接管理）；
返回值：
返回 frame->GetSize()，即成功接收的字节数；
如果失败（如缓冲区满、流控限制等），应返回负数（当前实现未体现错误处理）。
zhiy zeng
*/
int
...
/*
作用：将传入的数据包拆分为多个 QUIC Frame；
参数说明：
data: 接收到的原始 Packet，通常是一个包含多个 QUIC Frame 的完整 UDP 数据报；
返回值：
返回一个 vector<pair<Ptr<Packet>, QuicSubheader>>；
每个元素表示一个完整的 Frame 及其对应的 QUIC 子头部。
...
QuicL5Protocol::DisgregateRecv (Ptr<Packet> data)
{
  ...
  // 创建一个 vector 容器，保存解析后的 Frame 和对应的 QUIC 子头部
  ...
  // 逐个解析数据包中的 QUIC Frame
  for (uint32_t start = 0; start < dataSizeByte; )
    {
      QuicSubheader sub;
      data->RemoveHeader (sub); // 从 Packet 中移除并解析出一个 QUIC Subheader
      ...
      data->RemoveAtStart (sub.GetLength ()); // 移动 Packet 指针，准备下一次解析
      // 更新偏移量并保存结果: sub.GetSerializedSize() 是子头部本身的长度
      // sub.GetLength() 是数据部分的长度
      start += sub.GetSerializedSize () + sub.GetLength ();
      // 将解析出的 Frame 和对应的 QUIC Subheader 存入 disgregated 向量中
...
/*
作用：在当前协议实例维护的流列表中查找指定流 ID 的流对象；
参数说明：
streamId: 要查找的流 ID；
返回值：
如果找到匹配的流对象，返回 Ptr<QuicStreamBase>；
否则返回空指针 nullptr。
...
/*
作用：将 QUIC 协议实例绑定到指定的 NS-3 节点；
参数说明：
node: 指向 NS-3 中 Node 对象的智能指针；
返回值：
无返回值（void）。
...
/*
作用：将传入的连接 ID 赋值给协议对象内部的成员变量m_connectionId；
参数说明：
connId: 一个 64 位无符号整数，表示该连接的唯一标识符；
返回值：
无（void）。
...
/*
z作用：获取当前协议实例所绑定套接字的最大包大小；
返回值：
返回一个 16 位无符号整数，表示最大包大小（字节数）；
...
/*
作用：当本端接收到对端发送的传输参数（通常是在 Initial 或 Handshake 包中携带的）时，
调用此函数；
参数说明：
transportParameters: 一个 QuicTransportParameters 类型的对象，包含对端在握手阶段声明的传输参数；
返回值：
无（void）。
...
作用：触发连接的中止操作；
参数说明：
transportErrorCode: 表示中止连接的错误码（Transport Error Code），用于对端理解为何连接被中断；
reasonPhrase: 可读的字符串描述，可用于调试或日志记录；
返回值：
无（void）
...
/*
作用：更新所有已创建 QUIC 流（Stream）的最大接收数据量；
参数说明：
newMaxStreamData: 新的初始流控窗口大小（单位为字节）；
返回值：
无（void）。
...
/*
作用：汇总当前连接下所有流的可用发送窗口；
返回值：
返回一个 uint64_t 类型的值，表示当前连接整体的发送流控窗口大小（单位为字节）
