...
// 计算并返回当前流级最大可通告数据量的SendMaxStreamData方法, zhiy zeng
uint32_t
QuicStreamBase::SendMaxStreamData ()
{
  // m_recvSize: 已接收的数据量, 接收缓冲区可用空间: m_rxBuffer->Available ()
  return m_recvSize + m_rxBuffer->Available ();
}

// 设置流级流量控制窗口上限的 SetMaxStreamData 方法, zhiy zeng
void
...
// 设置流的发送状态, 仅在条件成立时才更新状态, zhiy zeng
void
...
// 设置流的节点, zhiy zeng
// 将当前流实例与 NS-3 中的 Node 对象绑定，明确流的归属
void
...
{
  NS_LOG_FUNCTION (this);
  m_streamId = streamId; // 设置流ID: 将输入的streamId赋值给成员变量m_streamId

  uint64_t mask = 0x00000003;

  // 推断流类型: 根据streamId的低2位 (掩码0x00000003) 自动设置流的类型
  // (双向 / 单向、客户端 / 服务器发起)
...
// 更新接收缓冲区的追踪函数, 用于监控接收缓冲区的状态变化
// 该函数会在接收缓冲区大小发生变化时被调用, zhiy zeng
void
