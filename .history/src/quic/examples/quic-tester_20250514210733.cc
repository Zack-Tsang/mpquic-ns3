...
//  LogComponentEnable ("PacketMetadata", log_precision);
  ...
  QuicHelper stack;
  stack.InstallQuic (nodes); // Install the QUIC stack on the nodes, zhiy zeng


  ///////////////////// Create the Internet stack and assign IP addresses /////////////////////
  ...
  /////////////////////// Create the QUIC echo client and server ///////////////////////
  QuicEchoServerHelper echoServer (9); // Create a QUIC echo server on port 9, zhiy zeng

  // Install the QUIC echo server on node 2, zhiy zeng
  ApplicationContainer serverApps = echoServer.Install (nodes.Get (1));
  serverApps.Start (Seconds (1.0)); // Start the server at 1 second, zhiy zeng
  serverApps.Stop (Seconds (1200.0)); // Stop the server at 1200 seconds, zhiy zeng

  // Install the QUIC echo client on node 1 on port 9, zhiy zeng
  QuicEchoClientHelper echoClient (interfaces.GetAddress (1), 9);
  // Set the maximum number of packets to send, 1 packet, zhiy zeng
  echoClient.SetAttribute ("MaxPackets", UintegerValue (1)); 
  // Set the send interval to 1 second, 1 second, zhiy zeng
  echoClient.SetAttribute ("Interval", TimeValue (Seconds (1.0)));
  //echoClient.SetAttribute ("MaxStreamData", UintegerValue (1024));

  // Install the QUIC echo client on node 1, zhiy zeng
  ApplicationContainer clientApps = echoClient.Install (nodes.Get (0));
  // Set the content of the message to be sent, "Hello World", zhiy zeng
  echoClient.SetFill (clientApps.Get (0), "Hello World");
  clientApps.Start (Seconds (2.0)); // Start the client at 2 seconds, zhiy zeng
  clientApps.Stop (Seconds (5.0)); // Stop the client at 5 seconds, zhiy zeng
  /////////////////////// Create the QUIC echo client and server ///////////////////////

  Packet::EnablePrinting (); // Enable packet printing, zhiy zeng
  Packet::EnableChecking (); // Enable packet checking, zhiy zeng

  std::cout << "\n\n#################### STARTING RUN ####################\n\n";
  Simulator::Run (); // Run the simulation, zhiy zeng
  std::cout
      << "\n\n#################### RUN FINISHED ####################\n\n\n";
  Simulator::Destroy (); // Destroy the simulation, zhiy zeng
