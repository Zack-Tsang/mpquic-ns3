...
main (int argc, char *argv[])
{
  ...
  cmd.AddValue ("maxPackets",
                "Total number of packets for application to send", maxPackets);
  ...
  ipv4.SetBase ("10.1.1.0", "255.255.255.0"); // Using 10.1.1.0/24 subnet, zhiy zeng
  Ipv4InterfaceContainer i = ipv4.Assign (devices);

  NS_LOG_INFO ("Create Applications.");

  ApplicationContainer sourceApps; // QUIC sender, zhiy zeng
  ApplicationContainer sinkApps; // QUIC receiver, zhiy zeng
  // multiple QUIC flows, zhiy zeng
  for (uint32_t iterator = 0; iterator < QUICFlows; iterator++)
    {
      uint16_t port = 10000 + iterator;
      // QUIC sender settings, zhiy zeng
      BulkSendHelper source  ("ns3::QuicSocketFactory",
                              InetSocketAddress (i.GetAddress (1), port));
      // Set the amount of data to send in bytes.  Zero is unlimited.
      source.SetAttribute ("MaxBytes", UintegerValue (maxBytes));
      sourceApps.Add (source.Install (nodes.Get (0)));
      // QUIC receiver settings, zhiy zeng
    PacketSinkHelper sink ("ns3::QuicSocketFactory",
                             InetSocketAddress (Ipv4Address::GetAny (), port));
      sinkApps.Add (sink.Install (nodes.Get (1)));
    }

  sinkApps.Start (Seconds (0.0)); // QUIC receiver starts at time 0.0, zhiy zeng
  sinkApps.Stop (Seconds (5)); // QUIC receiver stops at time 5.0, zhiy zeng
  // QUIC sender starts at time 1.0, zhiy zeng
  sourceApps.Start (Seconds (1));
  // QUIC sender stops at time 5.0, zhiy zeng
  sourceApps.Stop (Seconds (5));

  if (tracing) // Enable tracing, zhiy zeng 
    {
      AsciiTraceHelper ascii;
      // Generate .tr file for tracing, zhiy zeng
  ...
  // Use flow monitor to get statistics, zhiy zeng
